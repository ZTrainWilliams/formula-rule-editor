...
class VTULoader extends Loader {
	...
	parse(data) {
		...
		function parseXML(stringFile) {
			...
			function parseDataArray(ele, compressed) {
				// console.log('## parseDataArray', ele)
				let numBytes = 0;
				...
				if (ele.attributes.format === 'binary' && compressed) {
					...
					if (ele.attributes.type === 'Float32'
						|| ele.attributes.type === 'Float64'  // VTU
					) {
			...
			if (json.PolyData) {
			...
			} else if (json.UnstructuredGrid) { // VTU

				console.log('VTU add UnstructuredGrid!', json)

				const piece = json.UnstructuredGrid.Piece;
				const compressed = json.attributes.hasOwnProperty('compressor');


				const sections = ['PointData', 'Points', 'CellData', 'Cells'];
				let sectionIndex = 0;
				const numberOfSections = sections.length;
				...
				while (sectionIndex < numberOfSections) {
					...
					const section = piece[sectionName];
					...
					if (section && section.DataArray) {
						...
						if (sectionName == 'Points') {
							...
							const numberOfCells = parseInt( piece.attributes.NumberOfCells );
							if ( numberOfCells > 0 ) {
